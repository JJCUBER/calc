...
// TODO: Will be splitting function definitions in my header files out to cpp files.  I originally kept the definitions inside the header files, as I was only planning on including the header files in 'calc.cpp'; however, I have since started needing to reference the other headers from each other.
    ...
    static int addMissingFunctionGroupings(std::vector<Token*>& tokens)
    {
        ...
        for (int i = tokens.size() - 2; i >= 0; i--)
        {
            ...
            case TokenType::Operator:
                ...
                std::cout << "Error: UNCAUGHT Malformed Input - input has an operator other than '^' directly after a function: '" << ((Function*)tokens[i])->name << '_' << '\'';
                return -1;
            }
        }
